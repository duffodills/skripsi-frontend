...
import { updateDiaryEntry } from "lib/api";
import type { DiaryEntry } from "@/interfaces/api/ListsOfApiInterface";
...
export default function EditReviewModal({
  ...
  // State, auto-fill dengan data diary jika ada
  const [playedAt, setPlayedAt] = useState("");
  const [platform, setPlatform] = useState("PC");
  const [status, setStatus] = useState<"completed" | "in-progress" | "dropped">(
    "completed"
  );
  ...
  useEffect(() => {
    if (diary) {
      setPlayedAt(diary.played_at ? diary.played_at.slice(0, 10) : "");
      setPlatform(diary.platform || "PC");
      setStatus((diary.status as any) || "completed");
      setReview(diary.review || "");
  ...
  const handleSave = async () => {
    ...
    try {
      await updateDiaryEntry(diary.id, body, token!);
      onClose();
      onSuccess();
    } catch (e: any) {
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <DialogPanel className="w-full max-w-lg transform overflow-hidden rounded-lg bg-gray-800 p-6 text-left align-middle shadow-xl transition-all">
                ...
                {/* FORM (sama dengan AddReviewModal, pakai state di atas) */}
