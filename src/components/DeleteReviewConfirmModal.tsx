...
import { deleteDiaryEntry } from "lib/api";

interface DeleteDiaryConfirmModalProps {
  isOpen: boolean;
  onClose: () => void;
  diaryId: number;
  gameName: string;
...
export default function DeleteDiaryConfirmModal({
  isOpen,
  onClose,
  diaryId,
  gameName,
  onDeleted,
}: DeleteDiaryConfirmModalProps) {
  const { token } = useAuth();
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      await deleteDiaryEntry(diaryId, token);
      onDeleted();
    } catch (e: any) {
      setError(e.message || "Failed to delete diary entry.");
    } finally {
      setLoading(false);
    }
  };
            ...
            >
              <Dialog.Panel className="w-full max-w-md bg-gray-800 p-6 rounded-lg shadow-xl text-left">
                <Dialog.Title className="text-xl font-semibold text-white mb-4">
                  Delete diary entry for "{gameName}"?
                </Dialog.Title>
